...
template <typename CoreTypeA, typename CoreTypeB>
class NVRAMCacheObjectEx : public ICacheObject
{
public:
	std::variant<std::shared_ptr<CoreTypeA>, std::shared_ptr<CoreTypeB>> m_ptrCoreObject;

public:
	NVRAMCacheObjectEx(std::variant < std::shared_ptr<CoreTypeA>, std::shared_ptr<CoreTypeB>> ptrObj)
		: m_ptrCoreObject(ptrObj)
	{
		//const std::size_t n = sizeof...(CoreObjectType);
		//std::cout << typeid(CoreObjectType).name() << std::endl;
